from TrainForClassifyProb import ProbTrainer, BertForClassify, bert_processor,ProbDataset, tokenizer
import json
...
def change_data(data:list):
    ...
    for n in data:
        for m in n['replys']:
            prob = m['prob_label']
            new_data.append({'query': f"{n['query']}{tokenizer.cls_token}{m['reply']}", 'reply':m['reply'],
                             'label':m['label'], 'prob_label':int(10*prob)/10})
...
if __name__ == '__main__':
    ...
    trainer.train(batch_size=32, accumulation_steps=2, total_epoches=100,learning_rate=0.00007)
