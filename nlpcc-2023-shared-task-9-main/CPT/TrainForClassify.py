...
from modeling_cpt import CPTForConditionalGeneration, CPTConfig, CPTForSequenceClassification
from sklearn.metrics import classification_report

tokenizer = BertTokenizer.from_pretrained('/home/leiningjie/PycharmProjects/model/cpt/cpt-base')
...
class CptForClassify(nn.Module):
    def __init__(self, model_name):
        super(CptForClassify, self).__init__()
        self.cpt_classify = CPTForSequenceClassification.from_pretrained(model_name)
        self.loss_fct = nn.CrossEntropyLoss()

    def forward(self, encode_input):
        label = encode_input['label'].long()
        logits = self.cpt_classify(encode_input['query']['input_ids']).logits
...
class ProbTrainer:
    ...
    def evaluate_to_result(self, per_gpu_batch_size=10, checkpoint_path=None):
        """
        Evaluate testsets
        """
        if checkpoint_path is not None:
        ...
        with torch.no_grad():
            ...
            for batch in tqdm(eval_loader):
                ...
                predicts.append(predict.cpu().numpy().item())
                labels.append(batch['label'].cpu().numpy().item())
                text = batch['query'][0].split('[CLS]')
                query = text[0]
                resp = text[1]
                record = {'query':query, 'response':resp, 'predict':predict.cpu().numpy().item(), 'label':batch['label'].cpu().numpy().item()}
                records.append(record)
            result = classification_report(labels, predicts)
