...
def forward_step(forward_step_func, data_iterator, model, input_tensor, losses_reduced):
    """Forward step for passed-in model_base.
    ...
    unwrapped_model = unwrap_model(
        model, (torchDDP, LocalDDP, Float16Module, DeepSpeedEngine))
...
def forward_backward_no_pipelining(forward_step_func, data_iterator, model,
    ...
    with context_handler():
        for i in range(get_num_microbatches() - 1):
            ...
            if not forward_only:
                backward_step(optimizer, model, input_tensor, output_tensor,
                              output_tensor_grad)
            if args.deepspeed:
...
def forward_backward_pipelining_with_interleaving(forward_step_func, data_iterator, model,
                                                  optimizer, timers, forward_only):
    """Run interleaved 1F1B schedule (model_base split into model_base chunks), with
    ...
    def get_model_chunk_id(microbatch_id, forward):
        """Helper method to get the model_base chunk ID given the iteration number."""
    ...
    def forward_step_helper(microbatch_id):
        """Helper method to run forward step with model_base split into chunks
    ...
    def backward_step_helper(microbatch_id):
        """Helper method to run backward step with model_base split into chunks
        (run set_virtual_pipeline_model_parallel_rank() before calling
        backward_step())."""
