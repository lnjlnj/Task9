...
# Default name for the model_base parallel rng tracker.
_MODEL_PARALLEL_RNG_TRACKER_NAME = 'model_base-parallel-rng'


# Whether apply model_base parallelsim to checkpointed hidden states.
...
def gather_split_1d_tensor(tensor):
    """Opposite of above function, gather values from model_base parallel ranks."""
...
def model_parallel_cuda_manual_seed(seed):
    """Initialize model_base parallel cuda seed.

    This function should be called after the model_base parallel is
    initialized. Also, no torch.cuda.manual_seed should be called
    after this function. Basically, this is replacement for that
    function.
    Two set of RNG states are tracked:
        default state: This is for data parallelism and is the same among a
                       set of model_base parallel GPUs but different across
                       different model_base paralle groups. This is used for
                       example for dropout in the non-tensor-model_base-parallel regions.
        tensor-model_base-parallel state: This state is different among a set of model_base
                              parallel GPUs, but the same across data parallel
                              groups. This is used for example for dropout in
                              model_base parallel regions.
    """
    ...
    if torch.distributed.get_rank() == 0:
        print('> initializing model_base parallel cuda seeds on global rank {}, '
              'model_base parallel rank {}, and data parallel rank {} with '
              'model_base parallel seed: {} and data parallel seed: {}'.format(
...
class CheckpointFunction(torch.autograd.Function):
    """This function is adapted from torch.utils.checkpoint with
       two main changes:
           ...
           2) the states in the model_base parallel tracker are also properly
              tracked/set/reset.
    """
    @staticmethod
    def forward(ctx, run_function, *args):
        ...
        # Divide hidden states across model_base parallel group and only keep
...
def checkpoint(function, *args):
    """Checkpoint a model_base or part of the model_base.
