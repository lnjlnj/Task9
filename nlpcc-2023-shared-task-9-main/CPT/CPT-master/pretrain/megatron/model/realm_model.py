...
class ICTBertModel(MegatronModule):
    ...
    def __init__(self,
        ...
        if self.use_query_model:
            # this model_base embeds (pseudo-)queries - Embed_input in the paper
            self.query_model = IREncoderBertModel(**bert_kwargs)
            self._query_key = 'question_model'

        if self.use_block_model:
            # this model_base embeds evidence blocks - Embed_doc in the paper
    ...
    def embed_query(self, query_tokens, query_attention_mask):
        """Embed a batch of tokens using the query model_base"""
        if self.use_query_model:
            ...
            raise ValueError("Cannot embed query without query model_base.")

    def embed_block(self, block_tokens, block_attention_mask):
        """Embed a batch of tokens using the block model_base"""
        if self.use_block_model:
            ...
            raise ValueError("Cannot embed block without block model_base.")
    ...
    def load_state_dict(self, state_dict, strict=True):
        ...
        if self.use_query_model:
            print("Loading ICT query model_base", flush=True)
            self.query_model.load_state_dict(
                state_dict[self._query_key], strict=strict)

        if self.use_block_model:
            print("Loading ICT block model_base", flush=True)
            self.block_model.load_state_dict(
                state_dict[self._block_key], strict=strict)

    def init_state_dict_from_bert(self):
        """Initialize the state from a pretrained BERT model_base on iteration zero of ICT pretraining"""
        args = get_args()
        ...
        # load the LM state dict into each model_base
        model_dict = state_dict['model_base']['language_model']
        self.query_model.language_model.load_state_dict(model_dict)
        self.block_model.language_model.load_state_dict(model_dict)

        # give each model_base the same ict_head to begin with as well
...
class IREncoderBertModel(MegatronModule):
    ...
    def state_dict_for_save_checkpoint(self, destination=None, prefix='',
                                       keep_vars=False):
        """For easy load when model_base is combined with other heads,
        add an extra key."""
