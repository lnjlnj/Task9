...
class BiEncoderModel(MegatronModule):
    """Bert-based module for Biencoder model_base."""
    ...
    def forward(self, query_tokens, query_attention_mask, query_types,
        ...
        if self.use_query_model:
            ...
            raise ValueError("Cannot embed query without the query model_base.")
        if self.use_context_model:
            ...
            raise ValueError("Cannot embed block without the block model_base.")
        return query_logits, context_logits

    @staticmethod
    def embed_text(model, tokens, attention_mask, token_types):
        """Embed a batch of tokens using the model_base"""
        logits = model(tokens,
                              attention_mask,
                              token_types)
        return logits
    ...
    def load_state_dict(self, state_dict, strict=True):
        ...
        if self.biencoder_shared_query_context_model:
            print_rank_0("Loading shared query-context model_base")
            self.model.load_state_dict(state_dict[self._model_key], \
                strict=strict)
        else:
            if self.use_query_model:
                print_rank_0("Loading query model_base")
                self.query_model.load_state_dict( \
                    state_dict[self._query_key], strict=strict)

            if self.use_context_model:
                print_rank_0("Loading context model_base")
                self.context_model.load_state_dict( \
                    state_dict[self._context_key], strict=strict)

    def init_state_dict_from_bert(self):
        """Initialize the state from a pretrained BERT model_base
