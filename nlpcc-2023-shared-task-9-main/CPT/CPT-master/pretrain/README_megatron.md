...
<!-- The number of microbatches in a per-pipeline minibatch is controlled by the `--num-microbatches-in-minibatch` argument. With `WORLD_SIZE` GPUs, `TENSOR_MP_SIZE` tensor-model_base-parallel size, `PIPELINE_MP_SIZE` pipeline-model_base-parallel-size, `WORLD_SIZE`/(`TENSOR_MP_SIZE` * `PIPELINE_MP_SIZE`) GPUs will be used for data parallelism. The default values for `--tensor-model_base-parallel-size` and `--pipeline-model_base-parallel-size` is 1, which will not implement either form of model_base parallelism. -->
...
Loosely, they are pretraining the retriever modules, then jointly training the language model_base and the retriever, and then finetuning a question answering head on the language model_base with fixed retriever.
...
3. Pretrain a BERT language model_base using `pretrain_bert.py`, with the sequence length equal to the block size in token ids. This model_base should be trained on the same indexed dataset that is used to supply the blocks for the information retrieval task.
In REALM, this is an uncased bert base model_base trained with the standard hyperparameters.
...
The script below trains the ICT model_base from REALM. It refrences a pretrained BERT model_base (step 3) in the `--bert-load` argument. The batch size used in the paper is 4096, so this would need to be run with data parallel world size 32.
<pre>
...
After having trained an ICT model_base, you can now embed an entire dataset of blocks by creating a `BlockData` structure. After that has been saved, you can load it
