...
def process_batch(batch):
    """Process batch and produce inputs for the model_base."""
    args = get_args()
...
def _build_train_valid_dataloaders(train_dataset, valid_dataset):
    ...
    # to the actual batch size the model_base will see for this dataset.
...
def finetune(train_valid_datasets_provider, model_provider,
    ...
    # Build model_base, optimizer and learning rate scheduler.
    timers('model_base and optimizer').start()
    model, optimizer, lr_scheduler = setup_model_and_optimizer(model_provider)
    timers('model_base and optimizer').stop()
    ...
    if args.iteration == 0 and args.pretrained_checkpoint is not None:
        ...
        # This is critical when only model_base is loaded. We should make sure
    ...
    timers.log(['train/valid/test dataset/dataloder', 'callback function',
                'model_base and optimizer', 'pretrained checkpoint'])
    print_rank_0('training ...')

    # Finetune the model_base.
    if args.epochs > 0:
