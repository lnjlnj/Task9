...
"""Merge model_base parallel partitions."""
...
def get_model(model_type):
    ...
    if model_type == 'BERT':
        ...
        raise Exception('unrecognized model_base type: {}'.format(model_type))
...
def get_mp_merge_args(parser):
    ...
    group.add_argument('--model_base-type', type=str, required=True,
                       choices=['BERT', 'GPT', 'RACE', 'MNLI', 'QQP'],
                       help='Type of the mdoel.')
    group.add_argument('--target-pipeline-model_base-parallel-size', type=int, default=1,
                       help='Degree of pipeline model_base parallelism in output model_base.')
...
def main():
    ...
    if args.pipeline_model_parallel_size > 1:
        print("Checkpoints with pipeline model_base parallelism are not currently supported.")
        exit()
    ...
    print('\n merging model_base parallel partitions ...')
    ...
    # Full model_base.
    print('> building the full model_base ...')
    ...
    assert args.num_layers % args.pipeline_model_parallel_size == 0, \
        'num_layers must be divisible by target pipeline model_base parallel size'
        ...
        for rank in range(args.pipeline_model_parallel_size):
            ...
            for dst_name, partition_param in model.named_parameters():
                if dst_name == "word_embeddings.weight":
                    ...
                    # to source layer number (single-model_base layer number)
                    src_name = re.sub(layer_re, update_layer_num, dst_name)
                print(f" > copying {src_name} to {dst_name} in rank {rank}'s model_base")
    ...
    for rank, model in enumerate(partitions):
        mpu.initialize.set_pipeline_model_parallel_rank(rank)
        print(f"> saving rank {rank}'s model_base")
