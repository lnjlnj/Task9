...
class LogitsList:
    """A list of logits obtained from a finetuned PET model_base"""

    def __init__(self, score: float, logits: List[List[float]]):
        """
        Create a new LogitsList.

        :param score: the corresponding PET model_base's score on the training set
...
class PLMInputFeatures(InputFeatures):
    """A set of numeric input features for a model_base pretrained with a permuted language modeling objective."""
