...
def train(args, train_dataset, model, tokenizer):
    ...
    #     # set global_step to global_step of last saved checkpoint from model_base path
    #     try:
    ...
    for _ in train_iterator:
        ...
        for step, batch in enumerate(epoch_iterator):
            ...
            loss = outputs[0]  # model_base outputs are always tuple in transformers (see doc)
            # if isinstance(model_base, CPTForSequenceClassification):
            ...
            if (step + 1) % args.gradient_accumulation_steps == 0 or (
                ...
                if args.local_rank in [-1, 0] and (do_save
                    ...
                    logger.info("Saving model_base checkpoint to %s", output_dir)
...
def load_and_cache_examples(args, task, tokenizer, evaluate=False):
    ...
    if os.path.exists(cached_features_file) and not args.overwrite_cache:
        ...
        if task in ["mnli", "mnli-mm"] and args.model_type in ["roberta", "xlmroberta"]:
            # HACK(label indices are swapped in RoBERTa pretrained model_base)
...
def main():
    ...
    parser.add_argument(
        ...
        type=str,
        required=True,
        help="Path to pre-trained model_base or shortcut name selected in the list",
    )
    parser.add_argument(
        "--task_name",
        default=None,
    ...
    parser.add_argument(
        ...
        type=str,
        required=True,
        help="The output directory where the model_base predictions and checkpoints will be written.",
    )
    ...
    parser.add_argument(
        "--do_lower_case", type=str2bool, default=True, help="Set this flag if you are using an uncased model_base.",
    )
    ...
    # Load pretrained model_base and tokenizer
    if args.local_rank not in [-1, 0]:
        torch.distributed.barrier()  # Make sure only the first process in distributed training will download model_base & vocab
    ...
    # Saving best-practices: if you use defaults names for the model_base, you can reload it using from_pretrained()
    if False and args.do_train and (args.local_rank == -1 or torch.distributed.get_rank() == 0):
        # Create output directory if needed
        if not os.path.exists(args.output_dir) and args.local_rank in [-1, 0]:
            os.makedirs(args.output_dir)

        logger.info("Saving model_base checkpoint to %s", args.output_dir)
        # Save a trained model_base, configuration and tokenizer using `save_pretrained()`.
        ...
        # Good practice: save your training arguments together with the trained model_base
        torch.save(args, os.path.join(args.output_dir, "training_args.bin"))

        # Load a trained model_base and vocabulary that you have fine-tuned
    ...
    if args.do_predict and args.local_rank in [-1, 0]:
        ...
        for checkpoint in checkpoints:
            ...
            #     continue
            # model_base = AutoModelForSequenceClassification.from_pretrained(checkpoint)
